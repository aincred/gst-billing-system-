...
export default function App() {
  ...
  // --- NEW EFFECT TO FETCH CUSTOMERS ON LOAD ---
  useEffect(() => {
  ...
  // --- NEW HANDLER TO AUTOFILL FROM DB ---
  const handleCustomerSelect = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    const id = e.target.value;
    setSelectedCustomerId(id);
    if (!id) return; // If they select the placeholder, do nothing

    setIsLoadingCustomer(true);
    try {
      const res = await fetch(`/api/customers/${id}/last-invoice`);
      const data = await res.json();
      ...
      if (data.success) {
        ...
        if (data.customer) {
          setCustomer({
            ...
            state: data.customer.state || 'Jharkhand'
          });
        }
        
        // 2. Fill Products & Services
        if (data.items && data.items.length > 0) {
          const formattedItems = data.items.map((item: any) => ({
            id: Date.now() + Math.random(), // Unique temporary ID for the UI
      ...
      console.error("Error fetching last invoice data:", err);
    } finally {
  ...
  const handleCreateNew = () => {
    ...
    setItems([{ id: Date.now(), isHeading: false, description: '', qty: 1, rate: 0, gstRate: 18 }]);
    setDiscount(0);
    setSelectedCustomerId(''); // Reset dropdown
    setActiveTab('edit');
  };
        ...
        {activeTab === 'edit' && (
          <div className="space-y-6 print:hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                  <div className="flex items-center justify-between mb-4 text-indigo-600 border-b pb-2">
                    <div className="flex items-center space-x-2">
                      <Users className="w-5 h-5" />
                      <h2 className="text-lg font-semibold text-slate-800">Billed To (Customer)</h2>
                    </div>
                    {/* --- ADDED DROPDOWN UI HERE --- */}
                    <div className="flex items-center space-x-2">
                      <select
                        value={selectedCustomerId}
                        onChange={handleCustomerSelect}
                        disabled={isLoadingCustomer}
                        className="text-sm p-1.5 border border-slate-300 rounded outline-none focus:ring-2 focus:ring-indigo-200 bg-slate-50 text-slate-700"
                      >
                        <option value="">Fill from previous customer</option>
                        {customersList.map((c: any) => (
                          <option key={c.id} value={c.id}>{c.name}</option>
                        ))}
                      </select>
                      {isLoadingCustomer && <span className="text-xs text-indigo-500 animate-pulse">Loading...</span>}
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div>
